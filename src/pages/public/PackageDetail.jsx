...
import { getDestinationName, packageService } from '@/services/cmsService';
...
const PackageDetail = () => {
  ...
  return (
    <>
      <SEO title={pkg.seoTitle || pkg.title} description={pkg.seoDescription || pkg.overview} path={`/packages/${pkg.slug}`} image={pkg.gallery?.[0]} jsonLd={jsonLd} />
      <main>
        <section className="relative min-h-[520px] bg-slate-900 text-white">
          ...
          <div className="relative mx-auto max-w-7xl px-4 py-24">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <span className="rounded-full bg-white/15 px-4 py-2">{getDestinationName(pkg.destinationId)}</span>
        ...
        <section className="mx-auto grid max-w-7xl gap-10 px-4 py-16 lg:grid-cols-[1fr_420px]">
          ...
          <aside className="h-fit rounded-3xl border bg-white p-6 shadow-xl">
            ...
            <p className="mb-5 mt-2 text-sm text-slate-600">Inquiry will be stored in the mock lead database and later in Supabase.</p>
            <QuoteForm sourceType="package_inquiry" packageId={pkg.id} initialDestination={getDestinationName(pkg.destinationId)} compact />
